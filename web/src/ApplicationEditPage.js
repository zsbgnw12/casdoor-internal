    ...
    return <>
      ...
      {this.state.activeMenuKey === "ui-customization" && (
        <React.Fragment>
          ...
          <Row style={{marginTop: "20px"}}>
            <Col style={{marginTop: "5px"}} span={(Setting.isMobile()) ? 22 : 3}>
              {Setting.getLabel(i18next.t("application:Use Bing daily wallpaper"), i18next.t("application:Use Bing daily wallpaper - Tooltip"))} :
            </Col>
            <Col span={21}>
              <Switch
                checked={!!this.state.application.useBingBackground}
                onChange={checked => this.updateApplicationField("useBingBackground", checked)}
              />
            </Col>
          </Row>
          <Row>
